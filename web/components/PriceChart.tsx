...
const METRIC_ORDER = ["price", "per_sqm"] as const;
...
function ChartBody({ visible, seriesKey }: { visible: Series; seriesKey: string }) {
  const W = 720;
  const H = 240;
  ...
  // A 1-point series produced [0, 0, 0] and a 2-point one [0, 1, 1], so the
  // x-axis labels collided on duplicate React keys.
  const xLabelIndices = Array.from(
    new Set([0, Math.floor(xs.length / 2), xs.length - 1])
  );
    ...
    <>
      <div className="flex items-baseline gap-3 mb-2">
        <div className="text-2xl font-bold tabular-nums">
          {fmtMoney(last.value, visible.currency)}
        </div>
        <div
...
export function PriceChart({ points }: { points: ChartPoint[] }) {
  ...
  // The metric toggles used to render both options unconditionally. A condo
  // with sale-price but no sale-per_sqm therefore had a "Price / sqm" button
  // that selected a combination with no series — and the whole <section>,
  // toggles included, returned null, so only a reload brought the chart back.
  const effectivePeriod = periods.includes(activePeriod) ? activePeriod : periods[0];
  const metrics = useMemo(() => {
    const set = new Set(
      series.filter((s) => s.period === effectivePeriod).map((s) => s.metric)
    );
    return METRIC_ORDER.filter((m) => set.has(m));
  }, [series, effectivePeriod]);
  const effectiveMetric = metrics.includes(activeMetric) ? activeMetric : metrics[0];

  const visible = series.find(
    (s) => s.period === effectivePeriod && s.metric === effectiveMetric
  );
  ...
  return (
    <section className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5">
      ...
      {visible ? (
        <ChartBody
          visible={visible}
          seriesKey={`${effectivePeriod}-${effectiveMetric}`}
        />
      ) : (
        <div className="h-40 grid place-items-center text-xs text-zinc-500">
