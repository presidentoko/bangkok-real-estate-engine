  ...
  return (
    <main className="max-w-3xl mx-auto p-6">
      ...
      {rows.length === 0 ? (
        ...
        <>
          {/* Desktop table — 6 columns overflowed the viewport on mobile
              with no wrapper (found in the 2026-07-31 audit); hidden below
              sm in favor of the stacked card list, matching the pattern
              already used in developer/[slug], retiree/[city], best/[slug]. */}
                ...
                <li key={r.id} className="py-3">
                  <Link href={`/${lang}/condo/${r.slug ?? r.id}`} className="block space-y-1">
                    ...
                    <div className="pl-8 text-[11px] text-zinc-500">
                      {region} · {provLabel(r.province)}
                    </div>
                    <div className="pl-8 flex gap-3 text-xs tabular-nums">
                      <span className="text-zinc-400">{t.table.listings} {r.active_listings_count}</span>
                      <span className={`font-bold ${cls}`}>{t.table.dom} {t.domDays(dom)}</span>
                      <span className="text-zinc-500">{t.table.maxDom} {t.domDays(maxDom)}</span>
                    </div>
                  </Link>
                </li>
              );
