...
import { GLOSSARY, localizeTerm } from "@/lib/glossary";
...
export default async function GlossaryIndex({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const t = getDictionary(lang);
  const terms = GLOSSARY.map((g) => localizeTerm(g, lang));
  const setJsonLd = buildDefinedTermSetJsonLd({
    name: t.glossary.title,
    url: `${SEO_SITE_URL}/${lang}/glossary`,
    terms: terms.map((g) => ({ term: g.term, url: `${SEO_SITE_URL}/${lang}/glossary/${g.slug}` })),
  });
  ...
  return (
    <main className="max-w-3xl mx-auto p-6">
      ...
      />
      <h1 className="text-3xl font-bold mb-2 mt-2">{t.glossary.title}</h1>
