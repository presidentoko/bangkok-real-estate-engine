...
// Every valid term is prebuilt below (closed vocabulary, not a partial
// sample like condo/[slug]) — so any /glossary/<term> outside this list is
// genuinely nonexistent. Without this, Next.js renders the notFound()
// boundary for unknown params but still ships it with HTTP 200 (confirmed:
// on-demand ISR fallback pages don't propagate the 404 status correctly even
// though the body content is the not-found page — a long-standing Next.js
// App Router gap). `dynamicParams = false` makes Next 404 at the router
// level for unlisted params instead, before the page ever renders, which
// does return the correct status.
