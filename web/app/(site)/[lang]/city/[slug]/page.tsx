...
import { expandHeroUrl, shrinkHeroUrl, warnIfNearCacheCeiling } from "@/lib/condo-compact";
...
// Closed set — every city is prebuilt (see generateStaticParams below), so
// any /city/<slug> outside CITIES is genuinely nonexistent. Without this,
// Next renders the notFound() boundary for unlisted params but still ships
// it as HTTP 200 (on-demand ISR fallback doesn't propagate 404 status
// correctly — confirmed by reproducing locally). dynamicParams=false makes
// Next 404 at the router level instead, before the page ever renders.
...
function encodeCityCondos(rows: CondoSummary[]): CompactCityCondos {
  ...
  for (let i = 0; i < n; i++) {
    ...
    c.hero[i] = shrinkHeroUrl(r.hero_image_url); c.bubble[i] = r.bubble_index;
...
function decodeCityCondos(c: CompactCityCondos): CondoSummary[] {
  ...
  for (let i = 0; i < c.count; i++) {
    out[i] = {
      ...
      province: "", hero_image_url: expandHeroUrl(c.hero[i]), bubble_index: c.bubble[i],
