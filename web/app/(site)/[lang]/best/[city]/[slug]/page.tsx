...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: ogFor(lang, { title, description, url: `${SEO_SITE_URL}/${lang}/best/${cityObj.slug}/${slug}` }),
  };
}
          ...
          <table className="w-full text-sm hidden sm:table">
            <thead className="text-xs uppercase tracking-wider text-zinc-500 bg-zinc-900 border-b border-zinc-800">
              <tr>
                ...
                <th className="text-right px-4 py-3">{t.yield}</th>
                {mrr != null && <th className="text-right px-4 py-3">{t.spread}</th>}
                <th className="text-right px-4 py-3">{t.sale}</th>
                <th className="text-right px-4 py-3">{t.rent}</th>
                ...
                <li key={r.id} className="p-3">
                  <Link href={`/${lang}/condo/${r.slug}`} className="block space-y-2">
                    ...
                    <div className="flex items-baseline gap-3 pl-8 text-xs text-zinc-500 tabular-nums">
                      ...
                      <span>· {t.rent} {fmtTHB(r.avg_monthly_rent)}/mo</span>
                      {r.foreign_quota_inventory_pct != null && (
