...
export default async function DeveloperPage({
  ...
  const t = getDictionary(lang).developerTable;
        ...
        <table className="w-full text-sm hidden sm:table">
          <thead className="text-xs uppercase tracking-wider text-zinc-500 bg-zinc-900 border-b border-zinc-800">
            <tr>
              ...
              <th className="text-right px-4 py-3">{t.year}</th>
              <th className="text-right px-4 py-3">{t.units}</th>
              <th className="text-right px-4 py-3">{t.sale}</th>
              <th className="text-right px-4 py-3 hidden md:table-cell">{t.yield}</th>
              <th className="text-right px-4 py-3 hidden md:table-cell">{t.rating}</th>
              <th className="text-right px-4 py-3 hidden lg:table-cell">{t.retiree}</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r) => (
            ...
            <li key={r.id} className="p-3">
              <Link href={`/${lang}/condo/${r.slug}`} className="block space-y-1">
                ...
                <div className="flex gap-3 text-xs text-zinc-500 tabular-nums">
                  ...
                  {r.gross_yield_pct != null && <span>· {r.gross_yield_pct.toFixed(2)}% {t.yield}</span>}
                </div>
              </Link>
            </li>
          ))}
